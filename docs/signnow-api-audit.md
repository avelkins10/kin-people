...
| **Prefill smart fields** | `POST /document/{{document_id}}/integration/object/smartfields` | **Prefill smart fields**: same path. Body: `data: [{ field_name: value }, ...]`, `client_timestamp`. | ✅ Same. Called after create, before invite; field names must match template smart fields. |
| **Send invite** | `POST /document/{{documentId}}/invite` with `to` + `role_id` | **Field Invite**: `POST /document/{{document_id}}/invite` with `to: [{ email, role, role_id, order, reminder, expiration_days }]`. | We use legacy only; v2 path `v2/documents/.../invite` does not exist (404). We call **getDocumentRoles** then POST legacy with `to` + `role_id`. |
| **Get document roles** | `GET /v2/documents/{{documentId}}` (parse `roles[].unique_id`, `name`) | **Get document** returns `roles` with `unique_id`, `name`. | Used when v2 invite fails so we can send legacy invite with `role_id`. |
